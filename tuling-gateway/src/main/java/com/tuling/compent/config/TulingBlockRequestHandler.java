package com.tuling.compent.config;
...
public class TulingBlockRequestHandler extends DefaultBlockRequestHandler {
    ...
    private ErrorResult buildErrorResult(Throwable ex) {
        if(ex instanceof ParamFlowException) {
            return  new ErrorResult(HttpStatus.TOO_MANY_REQUESTS.value(),"block");
        }else if (ex instanceof DegradeException) {
            return new ErrorResult(HttpStatus.TOO_MANY_REQUESTS.value(),"fallback");
        }else{
            return new ErrorResult(HttpStatus.BAD_GATEWAY.value(),"gateway error");
        }
    }
